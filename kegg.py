...
import numpy as np
import pandas as pd
from data import ResponseDataloadersFactory, Source, GeneSet, get_pathways, get_gene_list
from Bio.KEGG.REST import *
from data import DATA_DIR, KEGGID_DIR, KEGG_GENE_DIR
import torch
...
def hierarchies_to_layers(hierarchies, layers=None, include_values=False, subset_from=None):
    if layers is None:
        layers = []
    ...
    if subset_from is not None:
        layer_from = np.intersect1d(layer_from, subset_from)
...
def construct_kegg_hierarchies(gene_set):
    genes = get_gene_list(gene_set)
    use_pathways_names = True
    kegg_pathways = get_pathways()
    keggId2symbol = kegg_pathways[2]
    ...
    gene_layer = hierarchies_to_layers(
        pathways, include_values=True, subset_from=genes)
    gene_kegg_subset = gene_layer[0]['from']
...
def keggid_pathways2symbol(keggid2symbol):
    pathway_files = os.listdir(KEGGID_DIR)
    for pathway_file in pathway_files:
        print(os.path.join(KEGGID_DIR, pathway_file))
        adjacency_list = pd.read_csv(os.path.join(KEGGID_DIR, pathway_file))
        adjacency_list['from'] = adjacency_list['from'].apply(
            lambda x: keggid2symbol[str(x)] if str(x) in keggid2symbol else str(x))
        adjacency_list['to'] = adjacency_list['to'].apply(
            lambda x: keggid2symbol[str(x)] if str(x) in keggid2symbol else str(x))
        out_file = os.path.join(KEGG_GENE_DIR, pathway_file)
        adjacency_list.to_csv(out_file, index=None)
